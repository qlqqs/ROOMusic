...
function App() {
  ...
  const [searchQuery, setSearchQuery] = useState(() => new URLSearchParams(window.location.search).get("q")?.trim() ?? "");
  const [releaseLoading, setReleaseLoading] = useState(false); const [releaseError, setReleaseError] = useState("");
  ...
  useEffect(() => {
    const onPopState = () => { const query = new URLSearchParams(window.location.search).get("q")?.trim() ?? ""; setSearchInput(query); setSearchQuery(query); };
    window.addEventListener("popstate", onPopState); return () => window.removeEventListener("popstate", onPopState);
  }, []);
  useEffect(() => { if (!session) return; const params = new URLSearchParams({ page: "1", page_size: "50" }); if (searchQuery) params.set("q", searchQuery); setReleaseLoading(true); setReleaseError(""); void requestApi(`/api/v1/releases?${params.toString()}`, decodeReleaseList).then((result) => { setReleases(result.items); setReleaseTotal(result.pagination.total); }).catch((error: unknown) => { setReleaseError(describeError(error)); }).finally(() => setReleaseLoading(false)); }, [session, scan?.status, searchQuery, releaseRetry]);
  ...
  function submitSearch(event: FormEvent) { event.preventDefault(); const query = searchInput.trim(); const url = new URL(window.location.href); if (query) url.searchParams.set("q", query); else url.searchParams.delete("q"); window.history.pushState({}, "", url); setSearchQuery(query); }
  function clearSearch() { setSearchInput(""); const url = new URL(window.location.href); url.searchParams.delete("q"); window.history.pushState({}, "", url); setSearchQuery(""); }
  if (setupRequired === null) return <main className="shell"><p role="status">正在加载...</p>{message && <p role="alert">{message}</p>}</main>;
  ...
  return <main className="shell"><p className="eyebrow">管理员 · {session.username}</p><h1>ROOMusic</h1><form onSubmit={registerRoot}><label>音乐目录<input placeholder="允许的音乐目录路径" value={libraryPath} onChange={(event) => setLibraryPath(event.target.value)} /></label><button type="submit">注册目录</button></form><h2>已注册目录</h2>{libraryRoots.length === 0 ? <p>尚未注册音乐目录</p> : <ul>{libraryRoots.map((root) => <li key={root.id}>{root.path}</li>)}</ul>}<button type="button" onClick={startScan} disabled={scan?.status === "running"}>扫描音乐库</button> <button type="button" onClick={() => void logout()}>退出登录</button>{scan && <p role="status">扫描状态：{scan.status}</p>}<h2>发行版本</h2><form onSubmit={submitSearch}><label>搜索发行版本<input value={searchInput} onChange={(event) => setSearchInput(event.target.value)} placeholder="标题、艺术家或曲目" /></label><button type="submit">搜索</button>{searchQuery && <button type="button" onClick={clearSearch}>清空</button>}</form>{releaseLoading ? <p role="status">正在加载发行版本...</p> : releaseError ? <p role="alert">{releaseError} <button type="button" onClick={() => setReleaseRetry((retry) => retry + 1)}>重试</button></p> : <><p>共 {releaseTotal} 个发行版本</p>{releases.length === 0 ? <p>暂无匹配的发行版本</p> : <ul>{releases.map((release) => <li key={release.id}><button type="button" onClick={() => void showRelease(release.id)}>{release.title} · {release.artist}</button></li>)}</ul>}</>}{selectedRelease && <section aria-label="发行版本详情"><h2>{selectedRelease.title}</h2><p>{selectedRelease.artist}</p>{selectedRelease.artwork && <img src={`/api/v1/artworks/${encodeURIComponent(selectedRelease.artwork.resource_id)}`} alt={`${selectedRelease.title} 封面`} width={selectedRelease.artwork.width} height={selectedRelease.artwork.height} />}{selectedRelease.media.map((medium) => <section key={medium.id} aria-label={`碟片 ${medium.position}`}><h3>碟片 {medium.position} · {medium.title}</h3>{medium.tracks.length === 0 ? <p>此碟片暂无曲目</p> : <ol>{medium.tracks.map((track) => <li key={track.id}>{track.position}. {track.title} · {track.artist} <small>来源：{track.source}</small></li>)}</ol>}</section>)}</section>}{message && <p role="alert">{message}</p>}</main>;
}
