...
- Good：管理员状态变更锁定同一组管理员行，session 撤销与 `disabled_at` 一起提交；
  两个并发禁用请求至多成功一个。
- Base：重复 POST 已停用目录返回 disabled；管理员随后使用 restore 端点显式恢复。
- Good：多碟来源按 `disc_number=1/2` 形成两个 Medium，重扫保持 Track ID。
- Bad：把 `role` 缺失解释为 admin；把 root 原始路径放入列表 JSON；unsupported
...
- 运行门禁：`npm run lint`、`npm run typecheck`、`npm run test`、
  ...
  `bash -n scripts/*.sh` 和 `docker compose config --quiet`（按改动取最小集合）。
